...
1. Положи `RpToolkit.dll` в папку плагинов сервера.
2. Запусти сервер один раз — модуль создаст `keystore.json`.
3. Зайди под ключом овнера (см. §3) и вызови `key_rotate_owner` — получишь
   свой личный ключ овнера.
4. Запиши полученный `hash` в конфиг в поле `owner_public_hash`
   (иначе после перезапуска плагина вернётся старое значение из конфига!).
5. В приложении открой TCP-соединение на порт `3091` и шли запросы (§5).
